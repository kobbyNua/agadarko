...
def patient_opd_panel(request):
	...
	'''
	regions=getRegion()
	return render(request,'dashboard/patients/opd-patient-records.html',{'title':'Patient OPD Records'.upper(),'page_title':'Patient Records','regions':regions,'path':'home'})
...
def create_patient_complaints_diagonsis(request):
	...
	user_id=request.POST['user_id']
	patient_complaints=request.POST['complaints']
	status_type=""
	msg=""
...
def edit_doctor_diagonsis(request):
	...
	diagnosis=request.POST['doctor_diagonsis']
...
def dispen_patient_dietary(request):
	...
	dispense_dietary=input_patient_dietry_request(patient_dietary_id,dietary_id,dispensed_status,quantity)
	msg=""
	status_type=""
