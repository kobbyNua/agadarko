...
from .controlview import create_hospital_details,check_super_user_status,get_staff,get_user_hospital_details,get_user_details,view_all_staffs,create_staff,edit_staff,change_staff_password
...
from .decorators import hospital_ddetails_set_up,allowed_user
		...
		if user is not None:
			...
			if request.user.is_superuser:
				if not request.user.groups.exists() :
					if check_super_user_status(request.user.id) == True:
						status_type+="success"
...
@allowed_user(allowed_roles=['Root','Administrator','Accountant','Nurse','Doctor','Dietary','Lab Technician'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Nurse'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Nurse','Dietary'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Doctor'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
#@allowed_user(allowed_roles=['Administrator','Doctor','Dietary','lab Technician'])
...
@login_required(login_url="/")
@allowed_user(allowed_roles=['Root','Administrator','Lab Technician'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Dietary'])
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Administrator','Root'])
def staff_management(request):
	'''
         admin only
	'''
	get_groups=Group.objects.all()
	all_staffs=view_all_staffs(request.user.id)
	user_info=get_user_hospital_details(request.user.id)
	if request.user.is_superuser == False:
		staff=get_staff(request.user.id)
	else:
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Accountant','Nurse','Doctor','Lab Technician','Dietary'])
def staff_user_management(request,staff_id):
	...
	if request.user.is_superuser == False:
		...
		staff=""
	return render(request,'dashboard/setting/staff-user-management.html',{'title':'staff management','staff_Id':staff,'hospital_id':user_info['hospital_id'],'user_id':user_info['user_id'],'staff_details':staff_details,'page_title':'staff user management details','path':'setting'})

@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator'])
def user_management(request):
	if request.user.is_superuser == False:
		staff=get_staff(request.user.id)
	else:
		staff=""	
	return render(request,'dashboard/setting/user-management.html',{'title':'staff management','staff_Id':staff,'page_title':'staff management','path':'staffs','page_title':'user management','path':'setting'})
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Accountant','Lab Technician'])
def laboratory_management(request):
	...
	if request.user.is_superuser == False:
		...
		staff=""	
	return render(request,'dashboard/setting/Laboratory-management.html',{'title':'Laboratory Management','staff_Id':staff,'user_id':user_info['user_id'],'hospital_id':user_info['hospital_id'],'lab_test':lab_test_lists,'page_title':'laboratory mgt.','path':'setting'})
...
def laboratory_test_management_details(request,lab_test_id):
	...
	if request.user.is_superuser == False:
		...
		staff=""		
	return render(request,'dashboard/setting/Laboratory-test-details.html',{'title':'Laboratory Test Management Details','staff_Id':staff,'hospital_id':user_info['hospital_id'],'user_id':user_info['user_id'],'lab_details':laboratory_test,'page_title':'lab test details management','path':'setting'})

@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Accountant','Dietary'])
def dietary_stocking(request):
	...
	if request.user.is_superuser == False:
		...
		staff=""
	return render(request,'dashboard/setting/dietary-stock.html',{'title':'Dietary Supplement Stocking','staff_Id':staff,'user_id':user_info['user_id'],'hospital_id':user_info['hospital_id'],'dietary':dietary_list,'restock':restock_dietary,'page_title':'dietary stocking management','path':'setting'})
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Accountant','Dietary'])
def dietary_stocking_view(request,dietary_id):
	...
	if request.user.is_superuser == False:
		...
		staff=""
	return render(request,'dashboard/setting/dietary-stock-view-edit.html',{'title':'dietary stocking history','staff_Id':staff,'user_id':user_info['user_id'],'dietary_stock':dietary_supplement_stock,'dietary_id':dietary_id,'dietary_details':dietary_supplement_stock_details,'page_title':'dietary stocking','path':'setting'})
...
def company(request):
	...
	if request.user.is_superuser == False:
		...
		staff=""
	return render(request,'dashboard/setting/company.html',{'title':'comapny agadarko','staff_Id':staff,'Region':region,'page_title':'company set up','path':'company'})
...
@login_required(login_url="/")
#@hospital_ddetails_set_up
@allowed_user(allowed_roles=['Root','Administrator','Accountant'])
def patient_payment_list_records(request):
	...
	if request.user.is_superuser == False:
		...
		staff=""
	return render(request,'dashboard/Accounts/payment.html',{'title':'patients payment Records'.upper(),'staff_Id':staff,'hospital_id':user_info['hospital_id'],'user_id':user_info['user_id'],"patient_record":patient_record,'page_title':'Patient Payment List','path':'payment list'})
...
def opd_charges_info(request):
	...
	return render(request,'dashboard/Accounts/opd_charges.html',{'title':'OPD Visit Charges','staff_Id':staff,'user_id':user_info['user_id'],'hospital_id':user_info['hospital_id'],'opd_charges':opd_charges,'opd_charges_update':opd_charges_update,'page_title':'OPD Charges','path':'account'})
...
def reports(request):
	...
	return render(request,'dashboard/Accounts/report.html',{'title':'Reports','staff_id':staff,'page_title':'Report Statememt','path':'account','daily_patients_bills':daily_medical_payment,'daily_opd_payments':daily_opd_payments})
	...
	for group in user_groups:
		...
		if group.name == "Administrator" or request.user.is_superuser == True:
			...
			if group.name == "Accountant":
				...
				for messages in range(len(notifys_bell)):
					print(notifys_bell[messages])
					notification_messages.append(
				    {"messages":"{} {}".format(notifys_bell[messages]['lab_request']['counts'],notifys_bell[messages]['lab_request']['message']),'counts':notifys_bell[messages]['lab_request']['counts']}
				  )
...
def view_discount(request):
	...
	return render(request,'dashboard/accounts/discounts-rate.html',{'title':'Discount Rates','staff_Id':staff,'all_discounts':discounts,'all_rates':get_all_rate(),'page_title':'Discount rates','path':'discount'})
...
def reports_dashboard(request):
	...
	return render(request,'dashboard/reports/finance/finance-dashboard.html',{'title':'Finance Report Dashboard'.title(),'staff_Id':staff})
